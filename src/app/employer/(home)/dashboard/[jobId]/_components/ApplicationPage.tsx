...
const Card: React.FC<CardProps> = ({ title, children, className = "" }) => {
  return (
    <div
      className={`w-full bg-white border border-neutral-100 p-6 rounded-2xl items-center gap-5 mx-10 text-center ${className}`}
    >
      <div className="flex justify-between px-2 py-3 rounded-xl">
        <div className="flex gap-4 items-center">
          <span className="text-4xl text-secondary-700 font-semibold max-md:text-lg">
            {title}
          </span>
        </div>
      </div>
          ...
          <div className="flex max-lg:flex-col w-full bg-secondary-200 border border-neutral-100 p-6 max-lg:px-2 max-md:py-4 justify-between rounded-2xl items-center gap-5 mx-16 max-md:mx-4 text-center">
            ...
            {resumes ? (
              ...
              >
                <Button
                  variant="primary"
                  className="flex items-center gap-[6px] max-w-[150px] justify-center bg-blue-500"
                >
          ...
          <Card title="Project Details">
            {projects && projects.length > 0 ? (
              projects.map((project: Project) => (
                <div
                  key={project._id}
                  className="flex flex-col max-md:gap-3 p-4 border border-neutral-100 rounded-xl my-2 text-left"
                >
                  ...
                  <div className="flex gap-4 items-center">
                    <Link
                      href={project.link}
                      target="_blank"
                      className="text-blue-500 text-lg max-md:text-xs"
                    >
                      View Project
                    </Link>
                  </div>
          ...
          <Card title="Experience">
            {experience && experience.length > 0 ? (
              experience.map((exp: Experience) => (
                <div
                  key={exp._id}
                  className="flex flex-col p-4 border border-neutral-100 rounded-xl my-2 text-left"
                >
            ...
            {interests && interests.length > 0 ? (
              <div className="flex gap-2 flex-wrap justify-center">
                {interests.map((interest: string) => (
                  <Chip key={interest} variant="close">
                    {interest}
                  </Chip>
                ))}
              </div>
